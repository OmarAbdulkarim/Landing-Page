...
- `omar.jpg` — **you must add this**: save your portrait photo into this folder with exactly this name. It appears in the hero title block ("Prepared by"). If the file is missing, an "OA" monogram shows instead — nothing breaks.
...
- `ai-in-construction.html` — AI Field Playbook: where AI earns its keep in project controls (claims drafting, code generation, program narratives, contract navigation, team training), where it fails, prompt patterns for planners, and the ground rules used for team training. Linked from the landing page's AI section.
